...
class NexusSecureStorage:
    ...
    def _load_index(self, password: str) -> dict:
        ...
        try:
            ...
            # Cryptographic failure (wrong password, tampered index)
            raise ValueError("Authentication failed. Invalid master password or corrupted registry.") from e
    ...
    def delete_file(self, password: str, file_uuid: str):
        ...
        if os.path.exists(file_path):
            try:
                ...
                print(f"[Vault] Cryptographic Shred warning: byte overwrite bypassed: {e}")
            os.remove(file_path)
